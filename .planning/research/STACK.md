...
The proposed stack is fundamentally sound and aligns with the 2025/2026 geospatial web platform consensus. Three adjustments are recommended below (pgvector over Milvus for MVP, Serwist over next-pwa, Celery over ARQ). The rest of the choices are validated as correct.
...
**The project's PROJECT.md chose Milvus for "semantic/multimodal retrieval."** However, research strongly suggests pgvector is the better choice for this project's scale and team size.

**Why pgvector wins for this project:**

1. **Scale mismatch.** The project has ~1,395 hydraulic structures. Even with documents, photos, OCR text, and satellite imagery patches, the total vector count will be in the tens of thousands — perhaps low hundreds of thousands. pgvector's sweet spot is 1-50M vectors. Milvus is designed for 100M-1B+ vectors. Using Milvus for thousands of vectors is "almost certainly more database than your problem deserves" (source: 2026 vector DB comparison article).

2. **Operational complexity.** Milvus Standalone requires a separate Docker container with etcd + MinIO dependencies. Milvus Distributed requires Kubernetes. pgvector is a single `CREATE EXTENSION` on the existing PostgreSQL. For a 2-3 person team, every additional service is operational debt.

3. **Hybrid search is achievable in pgvector.** The AI copilot needs "hybrid search over Postgres + Milvus." With pgvector, hybrid search becomes a single SQL query combining PostgreSQL full-text search (`tsvector @@ tsquery`) with vector similarity (`embedding <=> query_vector LIMIT k`). No cross-system orchestration needed. Results are in one transaction with provenance metadata.

4. **Real-world precedent.** GeoLens (open-source geospatial catalog) uses "PostgreSQL + PostGIS + pgvector — spatial and semantic in one database" as their entire stack. This is exactly the pattern this project needs.

5. **pgvector + pgvectorscale.** The pgvectorscale extension further extends performance past 50M vectors with competitive throughput at 99% recall. The "Postgres is too slow for vectors" complaint is outdated — it came from the old IVFFlat era. HNSW indexing changed this.
...
**Migration path if needed:** pgvector → Milvus Lite (embedded, for prototyping) → Milvus Standalone (Docker, production). All share the pymilvus API. Data export tools exist.
...
**Martin** (Rust, MapLibre project) is 2-3x faster than TiPG in benchmarks and is the "2026 default stack" recommendation for PostGIS tile serving. However:
...
**Hybrid option (if performance becomes an issue later):** Run TiPG for OGC API Features + Martin for high-traffic tile serving. Both read from the same PostGIS. This is the pattern used in production geostack deployments.
...
npx create-next-app@latest zhambyl-portal --typescript --app --turbopack

# Core UI
npm install maplibre-gl next-intl @serwist/next serwist

# State & data
npm install @tanstack/react-query zustand dexie

# Styling (Tailwind v4 is built into create-next-app)
npm install -D @tailwindcss/postcss tailwindcss

# Dev tools
npm install -D @types/maplibre-gl eslint prettier
```
...
# Database
pip install sqlalchemy[asyncio] geoalchemy2 asyncpg alembic

# Geospatial
pip install tipg titiler[pillow] rio-cogeo pystac-client
...
# Document processing
pip install pytesseract Pillow pdf2image  # OCR
pip install rasterio shapely  # Geospatial Python
```
...
services:
  ...
  tipg:        # ghcr.io/developmentseed/tipg:latest
  titiler:     # TiTiler app
```
...
The full stack runs these services via Docker Compose:
...
**8 services.** This is manageable for a 2-3 person team with Docker Compose. If pgvector replaces Milvus (recommended), this avoids adding etcd + Milvus + MinIO-for-Milvus (3+ more containers).
